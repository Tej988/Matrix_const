...
Spec §26/§27 ask for a React web app *and* a React Native + Expo app. Building both from
...
capture="environment">`, which is adequate. Web push on iOS needs iOS 16.4+ *and* the app
...
(Phases 12–14) is *designed* now and *built* later, once billing is decided.
...
2. **Summary documents are maintained by client-side transactions**, not triggers. Every
   ...
   `projects/{id}/summary/current` together. See `DATABASE.md` § *Summary maintenance* for
...
server-authored ones. An authenticated user with devtools can *attempt* arbitrary writes,
...
documented in `SECURITY.md` § *Bootstrapping the first owner* — a deliberate manual step,
not an oversight.
...
one. Under ADR-002, Firebase Storage is therefore likely *unavailable* rather than merely
...
2. **Files live in Google Drive**, in one folder the owner creates and shares with staff.
   We request the **`drive.file` scope**, which grants access *only to files our app
   creates* — the app can never read the rest of anyone's Drive. That narrow scope is
...
- **OAuth consent screen.** Until Google verifies the app, users see an "unverified app"
  warning. With fewer than 100 users we stay in *Testing* mode with staff listed as test
...
| A3 | The business is the **contractor**, raising bills *to* clients like Tata Project Limited. Stonede / Rakesh Rao is the client contact. | **High** — inverts the receivables model. Please confirm. |
