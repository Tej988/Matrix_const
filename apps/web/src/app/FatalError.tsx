...
export function FatalError({
  ...
  const message =
    error instanceof Error ? error.message : typeof error === 'string' ? error : t('errUnknownBody')
  ...
  return (
    <main className="mx-auto flex min-h-dvh max-w-2xl flex-col justify-center gap-5 p-6">
      ...
      <div className="text-sm text-slate-600 dark:text-slate-300">
        <p className="font-medium">{t('thingsWorthChecking')}</p>
        <ul className="mt-2 list-disc space-y-1 pl-5">
          <li>{t('fatalCheckSignIn')}</li>
          <li>
            {t('fatalCheckEnvDoes')} <code>apps/web/.env.local</code> {t('fatalCheckEnvMatch')}
          </li>
          <li>
            {t('fatalCheckSdk')} <code>npm ls firebase</code>
            {t('fullStop')}
          </li>
        </ul>
      </div>
