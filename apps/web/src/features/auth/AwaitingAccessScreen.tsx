    ...
    <main className="mx-auto flex min-h-dvh max-w-md flex-col justify-center gap-6 p-6">
      <div>
        <h1 className="text-2xl font-semibold text-slate-900 dark:text-slate-100">
          {disabled ? t('accessTurnedOff') : t('waitingForAccess')}
        </h1>
        <p className="mt-2 text-slate-600 dark:text-slate-300">
          {disabled ? t('accessDisabledBody') : t('awaitingAccessBody')}
        </p>
      </div>
      ...
      <dl className="divide-y divide-slate-200 rounded-lg border border-slate-200 text-sm dark:divide-slate-700 dark:border-slate-700">
        <div className="flex justify-between gap-4 px-4 py-3">
          <dt className="text-slate-500 dark:text-slate-400">{t('signedInAs')}</dt>
          <dd className="text-right text-slate-900 dark:text-slate-100">{firebaseUser.email}</dd>
        </div>
        <div className="px-4 py-3">
          <dt className="text-slate-500 dark:text-slate-400">{t('yourAccountId')}</dt>
      ...
      {!disabled && (
        <div className="flex flex-col gap-2">
          ...
          >
            {copied ? `${t('copied')} ✓` : t('copyMyDetails')}
          </button>
          <a
